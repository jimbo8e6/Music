...
import { getUserByUsername, getFollowing } from "@/lib/queries";
...
export default async function FollowingPage({
  ...
  const users = await getFollowing(profileUser.id);
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <h1 className="mt-2 text-xl font-semibold">Following</h1>
      </div>

      {users.length === 0 ? (
        <p className="text-mist-500 text-sm">Not following anyone yet.</p>
      ) : (
