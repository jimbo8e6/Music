...
import type { CommunityReview, CommentNode } from "@/lib/queries";

interface ReviewWithComments extends CommunityReview {
...
export function CommunityReviewsClient({ albumId }: { albumId: string }) {
  ...
  useEffect(() => {
    fetch(`/api/albums/${albumId}/reviews`)
      .then((r) => r.json())
      .then(setData)
      .catch(() => setData({ currentUserId: null, reviews: [] }));
  }, [albumId, refreshKey]);
          ...
          <div className="flex flex-wrap items-center gap-x-3 gap-y-1">
            ...
            <span className="text-mist-500 ml-auto text-xs">
              {review.listenedOn
                ...
                : `Logged ${formatRelative(new Date(review.createdAt))}`}
            </span>
          </div>
